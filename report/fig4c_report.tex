...
\shareRuleNeutralDropped{} (\shareRuleNeutralDroppedNRounds{} rounds). The treatment of neutral raters is carried through the
...
here; the difference is the treatment of neutral raters). $n=\oursNRoundsLarge$ rounds with a
...
\midrule
\fillin{ST5 model size} & \fillin{base, large, xl, xxl} & \fillin{large (\hyperref[app:rsq]{Appendix~A})} \\
\fillin{Neutral (Likert 4) raters} & \fillin{count as non-minority; drop the rater} & \fillin{count as non-minority (SM 5.4.1)} \\
...
neutral-rater treatments $\times$ \sensNOrders{} column orders when the split is by Likert rating,
plus \sensNModels{} model sizes $\times$ \sensNOrders{} column orders when the split is by the sign
of the score, where there are no neutral raters: \sensNRuns{} runs, \sensNRunsPerModel{} per
