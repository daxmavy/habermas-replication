...
% ---------------------------------------------------------------- code references
% Code references are permalinks pinned to \repocommitfull.
\newcommand{\repourl}{https://github.com/daxmavy/habermas-replication}
...
in which I vary these ambiguous details (\fillin{\hyperref[sec:sensitivity]{Sensitivity analysis}}).
...
We choose model \fillin{sentence-t5-large} as the most-likely model used in
the original analysis, because \fillin{its marginal $r^2$ of \oursMarginalRsqLarge{} is within
...
\fillin{Position-axis endpoints: the released statements write the negation as ``NOT''} & \fillin{as released; ``NOT'' lowercased, as in the SM example} & \fillin{as released (Table~\ref{tab:endpoints})} \\
\fillin{Column order: how a round's opinions are assigned to the regressors} & \fillin{order in the released data; random} & \fillin{data order} \\
\bottomrule
\end{tabular}
\end{table}
...
\fillin{One detail of that description can be read more than one way. The released position
...
\centering
\small
\caption{\fillin{Two worked examples of the endpoint construction: the questions asked in the most
...
\toprule
 & \fillin{example 1} & \fillin{example 2} \\
\midrule
\fillin{Question} & \fillin{\exampleAQuestion} & \fillin{\exampleBQuestion} \\
\fillin{Released affirming statement} & \fillin{\exampleAAffirming} & \fillin{\exampleBAffirming} \\
\fillin{Released negating statement} & \fillin{\exampleANegating} & \fillin{\exampleBNegating} \\
\midrule
\multicolumn{3}{l}{\fillin{\emph{Pinned reading: statements as released}}} \\
\fillin{Affirming endpoint} & \fillin{``\exampleAPinnedAff''} & \fillin{``\exampleBPinnedAff''} \\
\fillin{Negating endpoint} & \fillin{``\exampleAPinnedNeg''} & \fillin{``\exampleBPinnedNeg''} \\
\midrule
\multicolumn{3}{l}{\fillin{\emph{``NOT'' lowercased, as in the SM example}}} \\
\fillin{Affirming endpoint} & \fillin{``\exampleANotLowerAff''} & \fillin{``\exampleBNotLowerAff''} \\
\fillin{Negating endpoint} & \fillin{``\exampleANotLowerNeg''} & \fillin{``\exampleBNotLowerNeg''} \\
\bottomrule
\end{tabular}
\end{table}
...
\centering
\small
\caption{\fillin{The primary specification under sentence-t5-large under each reading of the
...
\toprule
 & \fillin{statements as released (pinned)} & \fillin{``NOT'' lowercased, as in the SM example} \\
\midrule
\fillin{Initial statements} & \fillin{\axisPinnedInitCand{} (\axisPinnedInitCandBootSE)} & \fillin{\axisNotLowerInitCand{} (\axisNotLowerInitCandBootSE)} \\
\fillin{Initial winner} & \fillin{\axisPinnedInitWin{} (\axisPinnedInitWinBootSE)} & \fillin{\axisNotLowerInitWin{} (\axisNotLowerInitWinBootSE)} \\
\fillin{Revised statements} & \fillin{\axisPinnedRevCand{} (\axisPinnedRevCandBootSE)} & \fillin{\axisNotLowerRevCand{} (\axisNotLowerRevCandBootSE)} \\
\fillin{Revised winner} & \fillin{\axisPinnedRevWin{} (\axisPinnedRevWinBootSE)} & \fillin{\axisNotLowerRevWin{} (\axisNotLowerRevWinBootSE)} \\
\midrule
\fillin{Fig.\ 4A $r$} & \fillin{\axisPinnedFigAr} & \fillin{\axisNotLowerFigAr} \\
\fillin{Marginal $r^2$} & \fillin{\axisPinnedMarginalRsq} & \fillin{\axisNotLowerMarginalRsq} \\
\bottomrule
\end{tabular}
\end{table}
...
\caption{\fillin{The paper's result against the sensitivity sweep. Orange: the paper's values (SM
